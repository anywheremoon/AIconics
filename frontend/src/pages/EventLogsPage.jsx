...
import { deleteEventLog, getEventLogs } from '../api/riskApi';
...
export default function EventLogsPage() {
    ...
    async function loadLogs() {
        ...
        try {
            ...
            setLogs([]);
            setError(
                requestError.message ?? '행동 로그를 불러오지 못했습니다.',
            );
        } finally {
            setLoading(false);
        }
    ...
    const filteredLogs = useMemo(() => {
        return logs
            .filter((log) => {
                const logUserId = String(log.user_id ?? '').toLowerCase();

                const searchUserId = appliedFilters.userId.trim().toLowerCase();

                const matchesUserId =
                    searchUserId === '' || logUserId.includes(searchUserId);
                ...
                const matchesStartDate =
                    startTime === null ||
                    (createdTime !== null && createdTime >= startTime);

                const matchesEndDate =
                    endTime === null ||
                    (createdTime !== null && createdTime <= endTime);
            ...
            .sort((firstLog, secondLog) => {
                const firstTime = new Date(firstLog.created_at ?? 0).getTime();
    ...
    async function handleDelete(log) {
        ...
        try {
            ...
            setLogs((previousLogs) =>
                previousLogs.filter((previousLog) => previousLog.id !== log.id),
            );
        } catch (deleteError) {
            console.error('행동 로그 삭제 실패:', deleteError);

            setError(deleteError.message ?? '행동 로그를 삭제하지 못했습니다.');
        } finally {
            setDeletingId(null);
        }
    }
    ...
    return (
        <main className="page-container">
            <div className="page-header">
                <div>
                    ...
                    <p className="page-description">
                        수집된 사용자 행동 데이터와 계산된 위험도를 확인합니다.
                    </p>
                </div>
            ...
            <div className="result-summary">
                ...
                <span>
                    조회 결과: <strong>{filteredLogs.length}</strong>건
                </span>
            </div>
